...
const Classes = () => {
  ...
  return (
    <div style={{backgroundColor: COLORS.bg_Colour, height: '100vh', display: 'flex', flexDirection: 'column'}} className='pb-3 fixed'>
      ...
      <div className='pt-2 px-4 pb-3'>
        ...
        <Card style={{backgroundColor: COLORS.bg_Colour}}>
          ...
          <label className="inline-block cursor-pointer absolute right-20">
            ...
            <div
              className={`relative w-25 h-8 rounded-full flex items-center justify-between px-2 text-sm font-bold
                ...
                ${isOn ? 'bg-gradient-to-l from-[#7B00FF] to-[#B200FF]' : 'bg-[#ebeff3] shadow-[5px_5px_4px_rgba(255,255,255,0.7),2px_2px_3px_rgba(189,194,199,0.75)_inset]'}
              `}
            >
              {isOn ? (
                <span style={{fontFamily:FONTS.heading_01.fontFamily}} className="text-white">ONLINE</span>
              ) : (
                <span style={{fontFamily:FONTS.heading_01.fontFamily}} className="absolute right-2 text-[#2a2a2a]">OFFLINE</span>
              )}
          ...
          <div className='grid xl:grid-cols-8 lg:grid-cols-5 md:grid-cols-4 items-start mb-2 px-6 gap-4'>
            <Button 
              style={{...FONTS.heading_07, color: activeTab === 'live' ? COLORS.white : undefined}}
              onClick={() => setActiveTab('live')}
              className={`px-5 min-w-[120px] rounded-[6px] cursor-pointer ${
                activeTab === 'live' 
            ...
            <Button 
              ...
              className={`px-5 min-w-[120px] cursor-pointer ${
                activeTab === 'upcoming'
                  ? 'bg-gradient-to-l from-[#7B00FF] to-[#B200FF] text-white shadow-[0px_2px_4px_0px_rgba(255,255,255,0.75)_inset,3px_3px_3px_0px_rgba(255,255,255,0.25)_inset,-8px_-8px_12px_0px_#7B00FF_inset,-4px_-8px_10px_0px_#B200FF_inset,4px_4px_8px_0px_rgba(189,194,199,0.75),8px_8px_12px_0px_rgba(189,194,199,0.25),-4px_-4px_12px_0px_rgba(255,255,255,0.75),-8px_-8px_12px_1px_rgba(255,255,255,0.25)]'
                  : 'bg-[#ebeff3] shadow-[5px_5px_4px_rgba(255,255,255,0.7),2px_2px_3px_rgba(189,194,199,0.75)_inset]'
              }`}
