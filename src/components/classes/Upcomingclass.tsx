...
const Upcomingclass = ({
	...
	const filteredClasses = showOnlineOnly
		? data.data.filter((classItem: any) => classItem)
		: data.data.filter((classItem: any) => classItem);
	...
	// Table headers
	const headers = ['Date', 'Topic', 'Join Link', 'Duration', 'Action'];
		...
		<div className='bg-[#ebeff3] min-h-[300px]'>
			<Card style={{ backgroundColor: COLORS.bg_Colour }}>
				...
				{paginatedData.length > 0 ? (
					paginatedData.map((classItem: any, index: any) => (
						<Card
							key={index}
						...
						>
							<table className='w-full'>
								<tbody>
									...
									>
										<td>{classItem?.createdAt.split('T')[0]}</td>
