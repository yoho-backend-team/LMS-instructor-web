...
const CourseList: React.FC = () => {
		...
		const fetchData = async () => {
			try {
				const params = {
					...
					courseid: '67d7ff20adc836d7f85fb99e',
				};
        ...
        <div className="bg-[#ebeff3] rounded-2xl p-6 shadow-[-4px_-4px_4px_rgba(255,255,255,0.7),_5px_5px_4px_rgba(189,194,199,0.75)]">
          ...
          <div className="flex items-start gap-4">
            <div className="w-50 h-50 rounded-lg bg-[#EBEFF3] flex items-center justify-center shadow-[-4px_-4px_4px_rgba(255,255,255,0.7),_5px_5px_4px_rgba(189,194,199,0.75)]">
              <img src={GetImageUrl(courseSelectData?.image) ?? undefined}alt="Course Icon" className="w-35 h-35 object-contain" />
            </div>

            <div className="flex-1 w-full">
              <div className="flex justify-between items-center mb-1">
                <h3 style={{ ...FONTS.heading_02 }}>{courseSelectData?.course_name}</h3>
              </div>
  ...
  <span className="text-sm text-gray-600 ml-1">{courseSelectData?.rating}</span>
</div>
              ...
              <div className="flex items-center gap-10 flex-wrap mt-8">
                <div className="flex items-start gap-2">
                  ...
                  <div className="flex flex-col leading-tight">
                    <span style={{ ...FONTS.heading_05 }} className='mb-1'>Duration</span>
                    <span style={{ ...FONTS.para_02 }}>{courseSelectData?.duration}</span>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-2">
                  ...
                  <div className="flex flex-col leading-tight">
                    <span style={{ ...FONTS.heading_05 }} className='mb-1'>Status</span>
  ...
  <div className="flex justify-between items-start mb-4">
    ...
    <span style={{ ...FONTS.para_02 }}>{courseSelectData?.coursemodules?.length} {courseSelectData?.coursemodules?.length == 1 ? 'Chapter' : 'Chapters'}</span>
  </div>

  <div className="flex flex-col gap-6">
    {courseSelectData?.coursemodules?.map((chapter:any, index:any) => (
...
/>
        ...
        <div className="flex flex-col items-end">
          <Button className="bg-[#ebeff3] text-black hover:bg-[#ebeff3] shadow-[3px_3px_5px_rgba(255,255,255,0.7),inset_2px_2px_3px_rgba(189,194,199,0.75)] text-sm">
            <span style={{ ...FONTS.heading_07 }}> Class Id  :  {chapter?.id}</span>
