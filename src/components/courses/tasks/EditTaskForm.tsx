...
const EditTaskForm = ({ task, onSave, onClose }: EditTaskFormProps) => {
  ...
  const studentSubmissions: StudentSubmission[] = task.answers || [];
  ...
  useEffect(() => {
    const allCompleted = studentSubmissions.every(submission => {
  ...
  const handleGradeChange = (submissionId: string, field: string, value: string) => {
    ...
    if (field === "status" && value === "completed") {
      const submission = studentSubmissions.find(sub => sub._id === submissionId);
      if (submission) {
  ...
  const handleSubmit = () => {
    ...
    const allGraded = updatedSubmissions.every(sub => sub.status === "completed");
