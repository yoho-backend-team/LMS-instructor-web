...
const EditTaskForm = ({ task, onSave, onClose }: EditTaskFormProps) => {
	...
	const currentItems = studentSubmissions?.slice(
		indexOfFirstItem,
		indexOfLastItem
	);
	...
	const [studentGrades, setStudentGrades] = useState<
		Record<string, { mark: string; remark: string; status: string }>
	>(
	...
	const handleDownloadAttachment = async (submission: StudentSubmission) => {
		if (!submission?.file) {
		...
		try {
			const fileUrl = GetImageUrl(submission.file);
			if (!fileUrl) {
			...
			let filename = `attachment_${submission.student}_${submission._id}`;
			...
			// Fallback method: Direct link opening
			try {
				const fileUrl = GetImageUrl(submission.file);
				if (fileUrl) {
	...
	return (
		<>
			<div className='fixed inset-0 backdrop-blur-sm bg-opacity-50 flex items-center justify-center z-50'>
				<div
					className='bg-[#f1f4f8] rounded-2xl shadow-xl w-full max-w-6xl mx-4 scrollbar-hide'
												...
												{currentItems?.map((submission: any, index: number) => (
													...
													>
														...
														<td className='px-4 py-3'>
															<select
																value={
																	studentGrades[submission?._id]?.status ||
																	submission?.status
																}
																onChange={(e) =>
																	handleGradeChange(
														...
														<td className='px-4 py-3'>
															<input
																type='number'
																value={
																	studentGrades[submission?._id]?.mark ||
														...
														<td className='px-4 py-3'>
															<textarea
																value={
																	studentGrades[submission._id]?.remark ||
																	submission?.remark
																}
																onChange={(e) =>
																	handleGradeChange(
						...
						<div className='flex justify-end gap-4 mt-6 p-3 rounded-lg w-full bg-gray-10'>
							...
							>
								{isUpdating
									? 'Updating...'
									: allStudentsCompleted
									? 'Complete Task & Update All Grades'
									: 'Update All Grades'}
							</Button>
						</div>
					</div>
				...
				<div className='fixed inset-0 backdrop-blur-sm bg-opacity-50 flex items-center justify-center z-50'>
					<div className='bg-white rounded-lg p-6 max-w-3xl w-full mx-4'>
						...
						<div className='p-4 bg-gray-100 rounded-lg text-center'>
							...
							{!selectedSubmission?.file && (
								<p className=''>No attachment available</p>
							)}
						</div>
